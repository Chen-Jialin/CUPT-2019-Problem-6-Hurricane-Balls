% 单个球 无滑动约束
clear;close all;clc;
syms m g mu R x y phi theta psi x1 y1 phi1 theta1 psi1 x2 y2 phi2 theta2 psi2 N ffx ffy fp fr N
...
M10 = -2 * R * (ffx * cos(phi) + ffy * sin(phi)) * cos(theta / 2)^2 + R * N * sin(theta);
M20 = 2 * R * (ffx * sin(phi) - ffy * cos(phi)) * cos(theta / 2)^2 + R * fp - 2 * R * fr;
M3 = -R * (ffx * sin(phi) - ffy * cos(phi)) * sin(theta);
M1 = M10 * cos(psi) + M20 * sin(psi);
M2 = -M10 * sin(psi) + M20 * cos(psi);
equ1 = 2 * m * x2 - fx;
equ2 = 2 * m * y2 - fy;
equ3 = 2 * m * (-theta2 * sin(theta) - theta1^2 * cos(theta)) - N + 2 * m * g;
equ4 = I1 * omega11 - (I2 - I3) * omega2 * omega3 - M1;
equ5 = I2 * omega21 - (I3 - I1) * omega3 * omega1 - M2;
equ6 = I3 * omega31 - (I1 - I2) * omega1 * omega2 - M3;
[x2,y2,phi2,theta2,psi2,N,ffx,ffy] = solve(vx1,vy1,equ1,equ2,equ3,equ4,equ5,equ6,x2,y2,phi2,theta2,psi2,N,ffx,ffy);